...
const BigNumbers: React.FC<BigNumbersProps> = (props) => {
    ...
    const fetchData = async () => {
      try {
        const currentTimeRangeToUrl = {
          ...
          'месяц': 'http://192.168.0.57:8000/calculate_last_month_oil_yield_plan/',
    ...
    const fetchData = async () => {
      try {
        const currentTimeRangeToUrl = {
          ...
          'месяц': 'http://192.168.0.57:8000/calculate_last_month_gas_yield_plan/',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://192.168.0.57:8000/calculate_benzin/'); // Replace with your API endpoint
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://192.168.0.57:8000/calculate_kerosin/'); // Replace with your API endpoint
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
  ...
  useEffect(() => {
    switch (activeCategory) {
      case 'нефтегазовая отрасль':
        setLabels([`Добыча нефти (тонн) план - ${Math.floor(oilPlan)} факт ${oilFact}`,
      ...
      case 'урановая промышленность':
        setLabels([/*лейблы для урановой промышленности*/]);
        break;
      default:
        setLabels([
  ...
  }, [activeCategory, oilPlan, oilFact, gasPlan, gasFact, benzin, kerosin, dt, mt]);
